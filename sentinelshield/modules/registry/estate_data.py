"""Gujarat government CCTV estate registry — cities, areas, owners, demo cams, and live feeds."""
...
SENTINEL_LIVE_CAMS = [
    {
        ...
        "status": "online",
        "live_url": "",
        "source": f"rtsp://pishundhameliya%40gmail.com:2PRZ-MGZQ-3REF@103.250.160.189:8554/stream/cam{number:02d}",
    }
    for number in range(1, 32)
]


def total_cameras() -> int:
    """Return indicative total camera count across Gujarat estate."""
    return sum(c["cameras"] for c in CITIES)


def sample_points(city: dict[str, Any], area: tuple, n: int = 6) -> list[dict[str, Any]]:
    """Generate deterministic sample government cameras for empty registry areas."""
