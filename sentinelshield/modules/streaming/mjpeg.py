...
from sentinelshield.modules.streaming.hw_accel import create_hw_videocapture, safe_release_capture
...
from sentinelshield.modules.vision.deblur import blur_box
...
def mjpeg_frame_generator(
    ...
    while live_stream_state.is_active and live_stream_state.current_camera_id == camera_id:
        ...
        try:
            ...
            try:
                if isinstance(path, str) and path.startswith("rtsp://") and hasattr(cv2, "CAP_FFMPEG"):
                    cap = cv2.VideoCapture(path, cv2.CAP_FFMPEG) if cv2 is not None else None
                else:
